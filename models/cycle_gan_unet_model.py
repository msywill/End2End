...
class CycleGANUnetModel(BaseModel):
    """
...
#         if self.opt.train_phase == 2 or self.opt.train_phase == 4:
        ...
        self.criterionSeg = DiceLoss(sigmoid=True)
    ...
    def forward_unet(self):
        ...
        self.fake_mask = self.u_net(fake_B) # 不影响GA的参数


    def backward_unet(self):
        self.loss_unet = self.criterionSeg(self.fake_mask, self.real_mask)
    ...
    def backward_G_and_U(self):
        ...
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_B) * lambda_B
        # segmentation loss
        self.loss_seg = self.criterionSeg(self.fake_low, self.real_mask) * lambda_2
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + \
                      self.loss_cycle_A + self.loss_cycle_B + \
